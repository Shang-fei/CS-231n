...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    sim = get_similarity_no_loop(text_features, image_features)
    pred_classes = [class_texts[i] for i in sim.argmax(dim=0)]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        sim = get_similarity_no_loop(text_features, self.image_features)
        top_indices = torch.topk(sim, dim=1, k=k).indices.tolist()
        print(top_indices)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
