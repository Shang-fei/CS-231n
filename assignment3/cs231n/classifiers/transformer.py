...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        captions = self.embedding(captions) # [N, T, V]
        captions = self.positional_encoding(captions)
        features = self.visual_projection(features) # [N, V]
        features = features.unsqueeze(1).repeat(1, T, 1) # [N, T, V]
     
        mask = torch.tril(torch.ones(T, T))
        output = self.transformer(captions, features, mask) # [N, T, V]
