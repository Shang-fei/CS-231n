...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        ####################################################################
        sqrt_alphas_bar = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        sqrt_one_minus_alphas_bar = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        x_start = (x_t - sqrt_one_minus_alphas_bar * noise) / sqrt_alphas_bar
        ####################################################################
        return x_start
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        # Look at the coeffs in `__init__` method and use the `extract` function.
        ####################################################################
        sqrt_alphas_bar = extract(self.sqrt_alphas_cumprod, t, x_start.shape)
        sqrt_one_minus_alphas_bar = extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
        pred_noise = (x_t - sqrt_alphas_bar * x_start) / sqrt_one_minus_alphas_bar 
        ####################################################################
        return pred_noise
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        #      and sample x_{t-1}.
        ##################################################################
        output = self.model(x_t, t, model_kwargs)
        ...
        if self.objective == "pred_noise":
            ...
            x_start = output
            # noise = self.predict_noise_from_start(x_start)
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        output = self.model(x_t, t, model_kwargs)
        loss = (loss_weight * (target - output) ** 2).mean()
        ####################################################################
