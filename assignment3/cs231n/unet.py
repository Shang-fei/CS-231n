...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        cond_x = self.forward(x, time, model_kwargs)
        uncond_kwargs = {**model_kwargs, "text_emb": None}
        uncond_x = self.forward(x, time, uncond_kwargs)
        x = (cfg_scale + 1) * cond_x - cfg_scale * uncond_x
        ##################################################################
